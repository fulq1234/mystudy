...
public class DocumentUtil {
    ...
    static Logger logger = Logger.getLogger(DocumentUtil.class);
     ...
     * @return
     */
    public static Document againCommonDocument(String url) {
        //获取连接
        Connection connection = Jsoup.connect(url);
        //设置connection参数
        setConnection(connection);
        //获得代理IP
        return getCommonDocument(connection);
    }
    /****
     * 请求一次的方法 不做重试
     *
    ...
    public static void main(String[] args) throws IOException {
       Document document=DocumentUtil.againCommonDocument("http://www.baidu.com");
    ...
    public static Document getDocumentByProxy(Connection connection, int maxTimes, int times) {
        ...
        ProxyIp ip = ProxyUtils.getProxyIp();
        connection.proxy(ip.getIp(), ip.getPort());
        try {
            return connection.get();
        } catch (IOException e) {
            try {
    ...
    public static Document getCommonDocument(Connection connection) {
        Document document = null;
        try {
            document = connection.get();
        } catch (Exception e) {
