...
<script>
  ...
  function getProxyIpById() {
      var id=$("#id").val();
      $.ajax({
          type: 'POST',
          url: '${ctx}/client/proxyIp/getProxyIpById',
          data: {"id":id},
          dataType:"json",
          success: function (result) {
              if(result.status=="0"){
                          ...
                          //普通输入框
                          $("#creatdTime").val(result.data.creatdTime);
                          //普通输入框
                          $("#updatedTime").val(result.data.updatedTime);
                          //多选框
                          if(result.data.testCheck!=''){
                              var checkArray=result.data.testCheck.split(",");
                              $.each(checkArray,function(i,item){
                                  if(item!=null && item!=""){
                                      $("input[name='testCheck'][value="+item+"]").attr("checked",true);
                                  }
                              })
                          };
                          //普通输入框
                          $("#testSelect").val(result.data.testSelect);
                          //单选框
