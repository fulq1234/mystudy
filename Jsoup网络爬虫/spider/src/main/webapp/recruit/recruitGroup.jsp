    ...
    <div class="x_content">
        <form class="form-horizontal form-label-left input_mask" role="form">
            <div class="form-group">
                ...
                <div class="col-md-3 col-sm-3 col-xs-12 form-group">
                    <label for="taskId">统计维度</label>
                    <select name="name" id="name" class="select2_single form-control">
                        <option value="companyType">公司类型</option>
                        <option value="monthlySalary">月薪</option>
                        <option value="workNature">年龄</option>
                        <option value="experience">工作经验</option>
                        <option value="eucation">学历要求</option>
                        <option value="rNum">招聘人数</option>
                        <option value="companySize">公司规模</option>
                    </select>
                </div>
            </div>
        </form>
...
<script>
   ...
   var table=$('#table').bootstrapTable({
        url: '${ctx}/client/recruit/statisticsByGroup',
   ...
   $("#search").click(function () {
    ...
    $("#export").click(function () {
        ...
        window.open("${ctx}/client/recruit/exportDataExcel?name="+name+"&dataType="+dataType+"&taskId="+taskId);
    });
    $("#generateEchart").click(function () {
        var name=$("#name").val();
        var dataType=$("#dataType").val();
        var taskId=$("#taskId").val();
        var url="${ctx}/recruit/recruitChart.jsp?name="+name+"&dataType="+dataType+"&taskId="+taskId+"&pageNumber="+pageNumber+"&pageSize="+pageSize;
        window.ajaxDialog({
            title:returnName(name)+"分组统计图",
            width:"1000px",
            height:"500px",
            url:url
        });
    ...
    function returnName(name) {
        if (name = 'companyType') {
            ...
            return "月薪";
        } else if (name = 'workNature') {
            return "年龄";
        } else if (name = 'experience') {
            return "工作经验";
        } else if (name = 'eucation') {
            return "学历要求";
        } else if (name = 'rNum') {
            return "招聘人数";
